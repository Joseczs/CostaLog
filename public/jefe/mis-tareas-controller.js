...
function cargarMisTareas(jefeUid) {
  ...
  onSnapshot(q, (snap) => {
    ...
    // ⚠️ Igual que en el dashboard: esta consulta collectionGroup con "where"
    // requiere un índice de Firestore con alcance "Collection group". Si no
    // existe, la consulta falla aquí sin mostrar nada en pantalla.
    console.error('Error cargando mis tareas (revisa si falta un índice):', error);
    document.getElementById('empty-state').style.display = 'block';
    document.getElementById('empty-state').innerHTML =
      `<span style="color:red;">Error: ${error.message}<br>
       Revisa la consola del navegador (F12) para ver el link de creación de índice, si aplica.</span>`;
  });
}
